...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (NSInteger i = 0; i < self.dataSourceArr.count; i++) {
        ...
        if (!btn.enabled) {
            //选择的全选
            if(indexPath.row==0){
                [btn setTitle:self.defaulTitleArray[i] forState:UIControlStateNormal];
            } else{
                [btn setTitle:self.dataSource[indexPath.row] forState:UIControlStateNormal];
            }
            //定义三大 title
            [self chooseTextColorAndImageName:indexPath button:btn];
        }
    ...
    !self.chooseConditionBlock?:self.chooseConditionBlock(_lastClickedBtn.tag-kViewTagConstant, self.dataSource[indexPath.row],currentTitleArr,indexPath.row);
...
- (void)chooseTextColorAndImageName:(NSIndexPath *)indexPath button:(UIButton *)button{
    // 改变颜色
    // 如果用户自定义的,就用自定义的, 反之使用CFDropDownMenuView.bundle自带的
    UIColor *textColor;
    NSString *imageName;
    if (indexPath.row == 0){
        textColor = CF_Color_TextDarkGrayColor;
        if (self.stateConfigDict[@"normal"]){
            imageName = self.stateConfigDict[@"normal"][1]==nil ? self.stateConfigDict[@"normal"][0] : self.stateConfigDict[@"normal"][1];
        }else{
            imageName = CFDrowMenuViewSrcName(@"灰箭头.png")?:CFDrowMenuViewFrameworkSrcName(@"灰箭头.png");
        }
        
    }else{
        
        textColor = self.stateConfigDict[@"selected"] == nil ? CF_Color_DefaultColor : self.stateConfigDict[@"selected"][0];
        
        //使用自定义的
        if (self.stateConfigDict[@"selected"]){
            if (self.stateConfigDict[@"selected"][1]){
                imageName = self.stateConfigDict[@"selected"][1];
            }else{
                NSString *str = [NSString stringWithFormat:@"%@.png", self.stateConfigDict[@"selected"][1]];
                imageName = CFDrowMenuViewSrcName(str)?:CFDrowMenuViewFrameworkSrcName(str);
            }
            
        }else{
            imageName = CFDrowMenuViewSrcName(@"天蓝箭头.png")?:CFDrowMenuViewFrameworkSrcName(@"天蓝箭头.png");
        }
        
    }
    
    [self changTintColorWithTintColor:textColor tintColorImgName:imageName ForButton:button];
}
